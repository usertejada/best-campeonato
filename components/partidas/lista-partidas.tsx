// components/partidas/lista-partidas.tsx
import { Calendar, Clock, MapPin, Trophy, Shield, CheckCircle2 } from "lucide-react";
...
interface ListaPartidasProps {
  partidas: Partida[];
  onFinalizar?: (partida: Partida) => void;
}
...
export function ListaPartidas({ partidas, onFinalizar }: ListaPartidasProps) {
          ...
          <div key={data} className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {partidasDoDia.map((partida) => (
                <div
                  key={partida.id}
                  className="bg-white rounded-[14px] border border-[#E5E7EB] shadow-[0_1px_3px_rgba(0,0,0,0.06)] p-4"
                >
                  ...
                  {partida.status === "ao_vivo" && (
                    <button
                      onClick={() => onFinalizar?.(partida)}
                      className="mt-3 w-full flex items-center justify-center gap-1.5 text-[13px] font-semibold text-[#4F6BED] bg-white border border-[#4F6BED] rounded-[10px] py-2 hover:bg-[#EFF6FF] transition-colors"
                    >
